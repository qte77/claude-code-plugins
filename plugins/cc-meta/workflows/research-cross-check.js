...
// args: { consumer, research, focus? } — absolute paths to the consumer repo and the research hub.
// On the skill->Workflow path `args` arrives as a JSON string, so parse defensively.
const A = typeof args === 'string' ? JSON.parse(args) : (args ?? {})
const CONSUMER = A.consumer
const RESEARCH = A.research
if (!CONSUMER || !RESEARCH) {
  throw new Error('research-cross-check: args.consumer and args.research (absolute repo paths) are required')
}
